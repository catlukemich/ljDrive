...
public class AirCraft extends AirCraftTables
{
	...
	static final /*SpriteID*/ int _aircraft_sprite[] = {
		0x0EB5, 0x0EBD, 0x0EC5, 0x0ECD, 0x0ED5, 0x0EDD, 0x0E9D, 0x0EA5, 0x0EAD, 0x0EE5, 0x0F05, 0x0F0D, 0x0F15, 0x0F1D,
		0x0F25, 0x0F2D, 0x0EED, 0x0EF5, 0x0EFD, 0x0F35, 0x0E9D, 0x0EA5, 0x0EAD, 0x0EB5, 0x0EBD, 0x0EC5
	};
	...
	public static int GetAircraftImage(final Vehicle v, int direction) {
		...
		if (Sprite.is_custom_sprite(spritenum)) {
			...
			if (sprite != 0)
				return sprite;
			spritenum = EngineTables2.orig_aircraft_vehicle_info[v.engine_type.id
				- Global.AIRCRAFT_ENGINES_INDEX].image_index;
		}
	...
	public static void DrawAircraftEngine(int x, int y, int engine, int image_ormod) {
		...
		if (Sprite.is_custom_sprite(spritenum)) {
			...
			if (0 == sprite)
				spritenum = EngineTables2.orig_aircraft_vehicle_info[engine
					- Global.AIRCRAFT_ENGINES_INDEX].image_index;
		}

		Gfx.DrawSprite(sprite | image_ormod, x, y);

		if ((Engine.AircraftVehInfo(engine).subtype & 1) == 0) { Gfx.DrawSprite(Sprite.SPR_ROTOR_STOPPED, x, y - 5); }
	}
	...
	static int CmdBuildAircraft(int x, int y, int flags, int p1, int p2) {
		...
		if (!Engine.IsEngineBuildable(p1, Vehicle.VEH_Aircraft))
		...
		if (!IsAircraftHangarTile(tile) || !tile.IsTileOwner(PlayerID.getCurrent()))
			return Cmd.CMD_ERROR;
	...
	static int CmdStartStopAircraft(int x, int y, int flags, int p1, int p2) {
		...
		if (v.type != Vehicle.VEH_Aircraft || !Player.CheckOwnership(v.owner))
			return Cmd.CMD_ERROR;

		if (Global._patches.allow_municipal_airports.get() && mAirport.MA_VehicleIsAtMunicipalAirport(v)
			&& !v.isStopped())
	...
	static int CmdSendAircraftToHangar(int x, int y, int flags, int p1, int p2) {
		...
		if (v.getCurrent_order().type == Order.OT_GOTO_DEPOT && p2 == 0) {
			...
			if (!st.IsValidStation() || st.airport_tile == null
				|| Airport.GetAirport(st.airport_type).nof_depots() == 0) {
				StationID station;
	...
	static int CmdChangeAircraftServiceInt(int x, int y, int flags, int p1, int p2) {
		Vehicle v;
		int     serv_int = Depot
			.GetServiceIntervalClamped(p2); /* Double check the service interval from the user-input */

		if (serv_int != p2 || !Vehicle.IsVehicleIndex(p1))
			return Cmd.CMD_ERROR;
	...
	static int CmdRefitAircraft(int x, int y, int flags, int p1, int p2) {
		...
		if (v.type != Vehicle.VEH_Aircraft || !Player.CheckOwnership(v.owner))
			return Cmd.CMD_ERROR;
		if (!CheckStoppedInHangar(v))
			return Cmd.return_cmd_error(Str.STR_A01B_AIRCRAFT_MUST_BE_STOPPED);

		avi = Engine.AircraftVehInfo(v.getEngine_type().id);

		/* Check cargo */
		if (new_cid.id > AcceptedCargo.NUM_CARGO || !Vehicle.CanRefitTo(v.getEngine_type(), new_cid))
			return Cmd.CMD_ERROR;
	...
	static void CheckIfAircraftNeedsService(Vehicle v) {
		...
		if (Global._patches.servint_aircraft == 0)
			return;
	...
	public static void OnNewDay_Aircraft(Vehicle v) {
		...
		int cost = (int) (Engine.AircraftVehInfo(v.getEngine_type().id).running_cost * Global._price.aircraft_running
			/ 364);
	...
	static void HelicopterTickHandler(Vehicle v) {
		...
		if (v.getCurrent_order().type == Order.OT_LOADING || v.isStopped()) {
			if (u.cur_speed != 0) {
				u.cur_speed++;
				if (u.cur_speed >= 0x80 && u.cur_image == Sprite.SPR_ROTOR_MOVING_3) { u.cur_speed = 0; }
			}
		} else {
			if (u.cur_speed == 0)
				u.cur_speed = 0x70;
		...
		if (spd == 0) {
			...
			u.tick_counter = 0;
			img            = u.cur_image + 1;
			if (img > Sprite.SPR_ROTOR_MOVING_3)
				img = Sprite.SPR_ROTOR_MOVING_1;
		} else {
			return;
		}
	...
	static int UpdateAircraftSpeed(Vehicle v) {
		...
		if (v.air.desired_speed > new_speed) { v.air.desired_speed = new_speed; }
		...
		if (v.isAircraftBroken())
			spd = Math.min(spd, v.getMax_speed() / 3 * Global._patches.aircraft_speed_coeff);
		...
		if ((v.air.state == Airport.LANDING || v.air.state == Airport.ENDLANDING)
			&& spd > 15 * Global._patches.aircraft_speed_coeff)
	...
	static int GetAircraftFlyingAltitude(Vehicle v) {
		...
		if (v.getMax_speed() > 37 * Global._patches.aircraft_speed_coeff) {
			maxz = 171;
			if (v.getMax_speed() > 74 * Global._patches.aircraft_speed_coeff) { maxz = 180; }
		}
	...
	static boolean AircraftController(Vehicle v) {
		...
		if (0 != (amd.flag & AirportMovingData.AMED_HELI_RAISE)) {
			...
			if (u.cur_speed > 32) {
				v.cur_speed = 0;
				if (--u.cur_speed == 32)
					v.SndPlayVehicleFx(Snd.SND_18_HELICOPTER);
			} else {
				u.cur_speed = 32;
		...
		if (0 != (amd.flag & AirportMovingData.AMED_HELI_LOWER)) {
			if (UpdateAircraftSpeed(v) >= 1) {
				...
				if (st.airport_type == Airport.AT_OILRIG)
					z += 54;
				if (st.airport_type == Airport.AT_HELIPORT)
					z += 60;
		...
		if (Math.abs(desired_dist - dist) < 10) {
			...
			v.air.desired_speed = Math.min(v.getMax_speed() * Global._patches.aircraft_speed_coeff,
				36 * Global._patches.aircraft_speed_coeff);
		} else {
			if (dist < desired_dist && v.queue_item != null) {
				...
				if (v.queue_item.queue.getPos(v) > 2)
					v.air.desired_speed = Math.min(v.getMax_speed() * Global._patches.aircraft_speed_coeff,
						15 * Global._patches.aircraft_speed_coeff);
				else
					v.air.desired_speed = v.air.desired_speed = Math.min(
						v.getMax_speed() * Global._patches.aircraft_speed_coeff,
						36 * Global._patches.aircraft_speed_coeff);
			} else {
				// Too far, speed up
		...
		if (v.queue_item != null) {
			if (v.air.state == Airport.FLYING && v.subtype == 0 && v.queue_item.queue.getPos(v) != 1) {
		...
		// Need exact position?
		if (0 == (amd.flag & AirportMovingData.AMED_EXACTPOS)
			&& dist <= (((amd.flag & AirportMovingData.AMED_SLOWTURN) != 0) ? 8 : 4))
			return true;
		...
		if (0 == (amd.flag & AirportMovingData.AMED_NOSPDCLAMP) && v.cur_speed > 12)
			v.cur_speed = 12;
		...
		if (0 != (amd.flag
			& (AirportMovingData.AMED_TAKEOFF | AirportMovingData.AMED_SLOWTURN | AirportMovingData.AMED_LAND)))
		...
		curz = z;
		if (v.queue_item != null) { curz = GetAircraftFlyingAltitude(v); }
	...
	static void HandleCrashedAircraft(Vehicle v) {
		...
		if (v.air.crashed_counter < 650) {
			if (BitOps.CHANCE16(1, 32)) {
				...
				SetAircraftPosition(v, v.getX_pos(), v.getY_pos(), v.z_pos);
				r = HAL.Random();
				v.CreateEffectVehicleRel(BitOps.GB(r, 0, 4) + 4, BitOps.GB(r, 4, 4) + 4, BitOps.GB(r, 8, 4),
	...
	static final Point[] smoke_pos = {
		new Point(5, 5), new Point(6, 0), new Point(5, -5), new Point(0, -6), new Point(-5, -5), new Point(-6, 0),
		new Point(-5, 5), new Point(0, 6)
	};
	...
	static void HandleAircraftSmoke(Vehicle v) {
		...
		if ((v.tick_counter & 0x1F) == 0) {
			v.CreateEffectVehicleRel(smoke_pos[v.direction].x, smoke_pos[v.direction].y, 2, Vehicle.EV_SMOKE);
		}
	}
	...
	static void ProcessAircraftOrder(Vehicle v) {
		...
		if (v.getCurrent_order().type == Order.OT_GOTO_DEPOT || v.getCurrent_order().type == Order.OT_LOADING) {
			if (v.getCurrent_order().type != Order.OT_GOTO_DEPOT || 0 == (v.getCurrent_order().flags & Order.OF_UNLOAD))
				return;
		}

		if (v.getCurrent_order().type == Order.OT_GOTO_DEPOT && (v.getCurrent_order().flags
			& (Order.OF_PART_OF_ORDERS | Order.OF_SERVICE_IF_NEEDED)) == (Order.OF_PART_OF_ORDERS
				| Order.OF_SERVICE_IF_NEEDED)
		...
		if (order.type == v.getCurrent_order().type && order.flags == v.getCurrent_order().flags
	...
	static void MaybeCrashAirplane(Vehicle v) {
		...
		prob = 0x10000 / 1500;
		if (st.airport_type == Airport.AT_SMALL && (0 != (Engine.AircraftVehInfo(v.getEngine_type().id).subtype & 2))
			&& !Global._cheats.no_jetcrash.value) {
			prob = 0x10000 / 20;
		}

		if (BitOps.GB(HAL.Random(), 0, 16) > prob)
			return;
	...
	static void AircraftEntersTerminal(Vehicle v) {
		...
		if (0 == (st.had_vehicle_of_type & Station.HVOT_AIRCRAFT)) {
			...
			flags = (v.owner.equals(Global.gs._local_player))
				? NewsItem.NEWS_FLAGS(NewsItem.NM_THIN, NewsItem.NF_VIEWPORT | NewsItem.NF_VEHICLE,
					NewsItem.NT_ARRIVAL_PLAYER, 0)
				: NewsItem.NEWS_FLAGS(NewsItem.NM_THIN, NewsItem.NF_VIEWPORT | NewsItem.NF_VEHICLE,
					NewsItem.NT_ARRIVAL_OTHER, 0);
			NewsItem.AddNewsItem(Str.STR_A033_CITIZENS_CELEBRATE_FIRST, flags, v.index, 0);
		}
		...
		if (old_order.type == Order.OT_GOTO_STATION && v.getCurrent_order().station == v.last_station_visited) {
			v.getCurrent_order().flags = (old_order.flags & (Order.OF_FULL_LOAD | Order.OF_UNLOAD)) | Order.OF_NON_STOP;
		}
			...
			if (BitOps.HASBIT(old_order.flags, Order.OFB_PART_OF_ORDERS)) {
				...
				if (v.owner.isLocalPlayer()) {
					...
					NewsItem.AddValidatedNewsItem(
						Str.STR_A014_AIRCRAFT_IS_WAITING_IN, NewsItem.NEWS_FLAGS(NewsItem.NM_SMALL,
							NewsItem.NF_VIEWPORT | NewsItem.NF_VEHICLE, NewsItem.NT_ADVICE, 0),
						v.index, 0, AirCraft::ValidateAircraftInHangar);
				}
			}
		}
	}
	...
	static void AircraftNextAirportPos_and_Order(Vehicle v) {
		...
		if (v.getCurrent_order().type == Order.OT_GOTO_STATION || v.getCurrent_order().type == Order.OT_GOTO_DEPOT)
	...
	static void AircraftEventHandler_InHangar(Vehicle v, final Airport airport) {
		...
		if (v.getCurrent_order().station == v.air.targetairport) {
			...
			if (v.subtype != 0) {
				if (!AirportFindFreeTerminal(v, airport))
					return; // airplane
			} else {
				if (!AirportFindFreeHelipad(v, airport))
					return; // helicopter
			}
	...
	static void AircraftEventHandler_HeliTakeOff(Vehicle v, final Airport airport) {
		...
		if (v.owner.isLocalPlayer() && (p.EngineHasReplacement(v.getEngine_type())
	...
	static void AircraftEventHandler_Flying(Vehicle v, final Airport airport) {
		...
		if (!(v.subtype == airport.getAccPlanes() || st.airport_tile == null
			...
			while (current != null) {
				if (current.heading == landingtype) {
					...
					if (v.subtype == 0 && st.helicopter_queue.getTop() == v && Global._patches.aircraft_queueing) {
						...
						if (v.subtype == 0) {
							can_land = false;
							if (st.helicopter_queue.getPos(v) != 1) { v.air.desired_speed = 0; }
						}
					}
	...
	static boolean AirportMove(Vehicle v, final Airport airport) {
		...
		if (v.air.pos >= airport.getNofElements()) {
			Global.DEBUG_misc(0, "position %d is not valid for current airport. Max position is %d", v.air.pos,
				airport.getNofElements() - 1);
		...
		if (current.heading == v.air.state) {
			...
			if (v.air.state != AirConstants.FLYING)
				v.air.previous_pos = prev_pos;
			return true;
		}
		...
		if (current.next_in_chain == null) {
			if (AirportSetBlocks(v, current, airport)) { v.air.pos = current.next_position; } // move to next position
			return retval;
		}
	...
	static boolean AirportSetBlocks(Vehicle v, AirportFTA current_pos, final Airport airport) {
		...
		if (airport.getLayoutItem(current_pos.position).block != next.block) {
			...
			if (current_pos.block == next.block)
			...
			while (temp != null) {
				if (temp.heading == 255) {
					...
					if (!st.hasAirportBlocks(temp.block)) {
						...
						if (FreeTerminal(v, group_start, group_end))
							return true;
					}
				} else {
	...
	static boolean AirportFindFreeHelipad(Vehicle v, final Airport airport) {
		...
		if (airport.helipads == null)
			return AirportFindFreeTerminal(v, airport);
	...
	private static void updateOneAirplaneOnNewStation(Station st, final Airport ap, Vehicle v) {
		GetNewVehiclePosResult gp = new GetNewVehiclePosResult();
	...
	static void DrawAircraftImage(final Vehicle v, int x, int y, int selection) {
		...
		if (v.subtype == 0)
			Gfx.DrawSprite(Sprite.SPR_ROTOR_STOPPED, x + 25, y + 5);
		if (v.index == selection) { Gfx.DrawFrameRect(x - 1, y - 1, x + 58, y + 21, 0xF, Window.FR_BORDERONLY); }
	}
	...
	public static void CcCloneAircraft(boolean success, TileIndex tile, int p1, int p2) {
		if (success)
			ShowAircraftViewWindow(Vehicle.GetVehicle(Global._new_aircraft_id));
	}
				...
				do {
					...
					if (e.isAvailableToMe()) {
						...
						if (BitOps.IS_INT_INSIDE(--pos, -w.vscroll.getCap(), 0)) {
							...
							DrawAircraftEngine(x + 29, y + 10, engine_id,
		...
		case WE_CLICK:
			...
			case 5: { /* build */
				...
				if (sel_eng != Engine.INVALID_ENGINE)
					Cmd.DoCommandP(TileIndex.get(w.window_number), sel_eng, 0, AirCraft::CcBuildAircraft,
						Cmd.CMD_BUILD_AIRCRAFT | Cmd.CMD_MSG(Str.STR_A008_CAN_T_BUILD_AIRCRAFT));
			}
				break;
	...
	static final Widget _new_aircraft_widgets[] = {
		...
		new Widget(Window.WWT_CAPTION, Window.RESIZE_NONE, 14, 11, 239, 0, 13, Str.STR_A005_NEW_AIRCRAFT,
			Str.STR_018C_WINDOW_TITLE_DRAG_THIS),
		new Widget(Window.WWT_MATRIX, Window.RESIZE_BOTTOM, 14, 0, 227, 14, 109, 0x401,
			Str.STR_A025_AIRCRAFT_SELECTION_LIST),
		new Widget(Window.WWT_SCROLLBAR, Window.RESIZE_BOTTOM, 14, 228, 239, 14, 109, 0x0,
			Str.STR_0190_SCROLL_BAR_SCROLLS_LIST),
		new Widget(Window.WWT_IMGBTN, Window.RESIZE_TB, 14, 0, 239, 110, 161, 0x0, Str.STR_NULL),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_TB, 14, 0, 114, 162, 173, Str.STR_A006_BUILD_AIRCRAFT,
			Str.STR_A026_BUILD_THE_HIGHLIGHTED_AIRCRAFT),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_TB, 14, 115, 227, 162, 173, Str.STR_A037_RENAME,
			Str.STR_A038_RENAME_AIRCRAFT_TYPE),
		new Widget(Window.WWT_RESIZEBOX, Window.RESIZE_TB, 14, 228, 239, 162, 173, 0x0, Str.STR_RESIZE_BUTTON),
		//new Widget(   WIDGETS_END),
	};

	static final WindowDesc _new_aircraft_desc = new WindowDesc(-1, -1, 240, 174, Window.WC_BUILD_VEHICLE, 0,
		WindowDesc.WDF_STD_TOOLTIPS | WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET
			| WindowDesc.WDF_UNCLICK_BUTTONS | WindowDesc.WDF_RESIZABLE,
		_new_aircraft_widgets, AirCraft::NewAircraftWndProc);
	...
	static void AircraftRefitWndProc(Window w, WindowEvent e) {
		...
		case WE_PAINT: {
			...
			if (w.as_refit_d().cargo != AcceptedCargo.CT_INVALID) {
				int cost = Cmd.DoCommandByTile(v.tile, v.index, w.as_refit_d().cargo, Cmd.DC_QUERY_COST,
		...
		case WE_CLICK:
			...
			case 4: /* refit button */
				if (w.as_refit_d().cargo != AcceptedCargo.CT_INVALID) {
					final Vehicle v = Vehicle.GetVehicle(w.window_number);
					if (Cmd.DoCommandP(v.tile, v.index, w.as_refit_d().cargo, null,
						Cmd.CMD_REFIT_AIRCRAFT | Cmd.CMD_MSG(Str.STR_A042_CAN_T_REFIT_AIRCRAFT)))
						w.DeleteWindow();
				}
				break;
			}
	...
	static final Widget _aircraft_refit_widgets[] = {
		new Widget(Window.WWT_CLOSEBOX, Window.RESIZE_NONE, 14, 0, 10, 0, 13, Str.STR_00C5, Str.STR_018B_CLOSE_WINDOW),
		new Widget(Window.WWT_CAPTION, Window.RESIZE_NONE, 14, 11, 239, 0, 13, Str.STR_A03C_REFIT,
			Str.STR_018C_WINDOW_TITLE_DRAG_THIS),
		new Widget(Window.WWT_IMGBTN, Window.RESIZE_NONE, 14, 0, 239, 14, 145, 0x0,
			Str.STR_A03E_SELECT_TYPE_OF_CARGO_FOR),
		new Widget(Window.WWT_IMGBTN, Window.RESIZE_NONE, 14, 0, 239, 146, 167, 0x0, Str.STR_NULL),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_NONE, 14, 0, 239, 168, 179, Str.STR_A03D_REFIT_AIRCRAFT,
			Str.STR_A03F_REFIT_AIRCRAFT_TO_CARRY),
		//{   WIDGETS_END},
	};

	static final WindowDesc _aircraft_refit_desc = new WindowDesc(
		-1, -1, 240, 180, Window.WC_VEHICLE_REFIT, Window.WC_VEHICLE_VIEW, WindowDesc.WDF_STD_TOOLTIPS
			| WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET | WindowDesc.WDF_UNCLICK_BUTTONS,
		_aircraft_refit_widgets, AirCraft::AircraftRefitWndProc);
	...
	static void AircraftDetailsWndProc(Window w, WindowEvent e) {
		...
		case WE_PAINT: {
			...
			/* Draw running cost */
			{
				...
				Global.SetDParam(3, ((int) Global._price.aircraft_running)
					* Engine.AircraftVehInfo(v.getEngine_type().id).running_cost >> 8);
			...
			/* Draw service interval text */
			{
				...
				Gfx.DrawString(13, 103, Global._patches.servint_ispercent ? Str.STR_SERVICING_INTERVAL_PERCENT
					: Str.STR_883C_SERVICING_INTERVAL_DAYS, 0);
			}
...

			{
				...
				do {
					if (v.subtype <= 2) {
						...
						Gfx.DrawString(60, y, (u.getCargo_cap() != 0) ? Str.STR_A019_CAPACITY : Str.STR_A01A_CAPACITY,
							0);
						y += 14;
					}
		...
		case WE_CLICK: {
			...
			case 2: /* rename */
				...
				Global.SetDParam(0, v.unitnumber.id);
				MiscGui.ShowQueryString(new StringID(v.string_id), new StringID(Str.STR_A030_NAME_AIRCRAFT), 31, 150,
					w.getWindowClass(), w.window_number);
				break;
			/*	
			case 5: // increase int 
			...
			case 6: // decrease int 
				...
				Cmd.DoCommandP(v.tile, v.index, mod, null,
					Cmd.CMD_CHANGE_AIRCRAFT_SERVICE_INT | Cmd.CMD_MSG(Str.STR_018A_CAN_T_CHANGE_SERVICING));
				break;
			}
		}
			break;
	...
	static final Widget _aircraft_details_widgets[] = {
		new Widget(Window.WWT_CLOSEBOX, Window.RESIZE_NONE, 14, 0, 10, 0, 13, Str.STR_00C5, Str.STR_018B_CLOSE_WINDOW),
		new Widget(Window.WWT_CAPTION, Window.RESIZE_NONE, 14, 11, 349, 0, 13, Str.STR_A00C_DETAILS,
			Str.STR_018C_WINDOW_TITLE_DRAG_THIS),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_NONE, 14, 350, 389, 0, 13, Str.STR_01AA_NAME,
			Str.STR_A032_NAME_AIRCRAFT),
		...
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_NONE, 14, 0, 10, 102, 107, Str.STR_0188,
			Str.STR_884D_INCREASE_SERVICING_INTERVAL),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_NONE, 14, 0, 10, 108, 113, Str.STR_0189,
			Str.STR_884E_DECREASE_SERVICING_INTERVAL),
	...
	static final WindowDesc _aircraft_details_desc = new WindowDesc(
		-1, -1, 390, 114, Window.WC_VEHICLE_DETAILS, Window.WC_VEHICLE_VIEW, WindowDesc.WDF_STD_TOOLTIPS
			| WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET | WindowDesc.WDF_UNCLICK_BUTTONS,
		_aircraft_details_widgets, AirCraft::AircraftDetailsWndProc);
	...
	static final Widget _aircraft_view_widgets[] = {
		new Widget(Window.WWT_CLOSEBOX, Window.RESIZE_NONE, 14, 0, 10, 0, 13, Str.STR_00C5, Str.STR_018B_CLOSE_WINDOW),
		new Widget(Window.WWT_CAPTION, Window.RESIZE_RIGHT, 14, 11, 237, 0, 13, Str.STR_A00A,
			Str.STR_018C_WINDOW_TITLE_DRAG_THIS),
		...
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_RTB, 14, 0, 237, 104, 115, 0x0,
			Str.STR_A027_CURRENT_AIRCRAFT_ACTION),
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_LR, 14, 232, 249, 14, 31, 0x2AB,
			Str.STR_A029_CENTER_MAIN_VIEW_ON_AIRCRAFT),
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_LR, 14, 232, 249, 32, 49, 0x2AF,
			Str.STR_A02A_SEND_AIRCRAFT_TO_HANGAR),
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_LR, 14, 232, 249, 50, 67, 0x2B4,
			Str.STR_A03B_REFIT_AIRCRAFT_TO_CARRY),
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_LR, 14, 232, 249, 68, 85, 0x2B2,
			Str.STR_A028_SHOW_AIRCRAFT_S_ORDERS),
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_LR, 14, 232, 249, 86, 103, 0x2B3,
			Str.STR_A02B_SHOW_AIRCRAFT_DETAILS),
		new Widget(Window.WWT_PUSHIMGBTN, Window.RESIZE_LR, 14, 232, 249, 32, 49, Sprite.SPR_CLONE_AIRCRAFT,
			Str.STR_CLONE_AIRCRAFT_INFO),
	...
	static void AircraftViewWndProc(Window w, WindowEvent e) {
		...
		case WE_PAINT: {
			...
			if (v.isStopped() && IsAircraftHangarTile(v.tile)) { disabled = 0; }
			...
			Gfx.DrawSprite((v.isStopped()) ? Sprite.SPR_FLAG_VEH_STOPPED : Sprite.SPR_FLAG_VEH_RUNNING, 2,
				w.getWidget(5).top + 1);
			Gfx.DrawStringCenteredTruncated(w.getWidget(5).left + 8, w.getWidget(5).right, w.getWidget(5).top + 1,
				new StringID(str), 0);
		...
		case WE_CLICK: {
			...
			switch (e.widget) {
			case 5: /* start stop */
				Cmd.DoCommandP(v.tile, v.index, 0, null,
					Cmd.CMD_START_STOP_AIRCRAFT | Cmd.CMD_MSG(Str.STR_A016_CAN_T_STOP_START_AIRCRAFT));
				break;
			...
			case 7: /* goto hangar */
				Cmd.DoCommandP(v.tile, v.index, 0, null,
					Cmd.CMD_SEND_AIRCRAFT_TO_HANGAR | Cmd.CMD_MSG(Str.STR_A012_CAN_T_SEND_AIRCRAFT_TO));
				break;
			case 8: /* refit */
			...
			case 11:
				/* clone vehicle */
				Cmd.DoCommandP(v.tile, v.index, Global._ctrl_pressed ? 1 : 0, AirCraft::CcCloneAircraft,
					Cmd.CMD_CLONE_VEHICLE | Cmd.CMD_MSG(Str.STR_A008_CAN_T_BUILD_AIRCRAFT));
				break;
			}
		}
			break;
	...
	static final WindowDesc _aircraft_view_desc = new WindowDesc(-1, -1, 250, 116, Window.WC_VEHICLE_VIEW, 0,
		WindowDesc.WDF_STD_TOOLTIPS | WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET
			| WindowDesc.WDF_UNCLICK_BUTTONS | WindowDesc.WDF_STICKY_BUTTON | WindowDesc.WDF_RESIZABLE,
		_aircraft_view_widgets, AirCraft::AircraftViewWndProc);
	...
	static void DrawAircraftDepotWindow(Window w) {
		...
		w.disabled_state = tile.IsTileOwner(Global.gs._local_player) ? 0 : ((1 << 4) | (1 << 7) | (1 << 8));
		...
		while (ii.hasNext()) {
			...
			if (v.type == Vehicle.VEH_Aircraft && v.subtype <= 2 && v.isHidden()
		...
		while (ii1.hasNext()) {
			...
			if (v.type == Vehicle.VEH_Aircraft && v.subtype <= 2 && v.isHidden() && v.tile.getTile() == tile.getTile()
	...
	static int GetVehicleFromAircraftDepotWndPt(final Window w, int x, int y, Vehicle[] veh) {
		...
		while (ii.hasNext()) {
			Vehicle v = ii.next();
			if (v.type == Vehicle.VEH_Aircraft && v.subtype <= 2 && v.isHidden() && v.tile.tile == tile && --pos < 0) {
				veh[0] = v;
				if (xm >= 12)
					return 0;
				if (ym <= 12)
	...
	static void AircraftDepotClickAircraft(Window w, int x, int y) {
		...
		case 0: // start dragging of vehicle
			if (v[0] != null) {
				...
				w.SetWindowDirty();
				ViewPort.SetObjectToPlaceWnd(
					Sprite.SPRITE_PALETTE(Sprite.PLAYER_SPRITE_COLOR(v[0].owner)) + GetAircraftImage(v[0], 6), 4, w);
			}
			break;
		...
		case -2: // click start/stop flag
			Cmd.DoCommandP(v[0].tile, v[0].index, 0, null,
				Cmd.CMD_START_STOP_AIRCRAFT | Cmd.CMD_MSG(Str.STR_A016_CAN_T_STOP_START_AIRCRAFT));
			break;
	...
	static void HandleCloneVehClick(final Vehicle v, final Window w) {
		if (v == null || v.type != Vehicle.VEH_Aircraft)
			return;

		Cmd.DoCommandP(TileIndex.get(w.window_number), v.index, Global._ctrl_pressed ? 1 : 0, AirCraft::CcCloneAircraft,
			Cmd.CMD_CLONE_VEHICLE | Cmd.CMD_MSG(Str.STR_A008_CAN_T_BUILD_AIRCRAFT));
	...
	static void AircraftDepotWndProc(Window w, WindowEvent e) {
		...
		case WE_DRAGDROP:
			...
			case 5: {
				...
				if (GetVehicleFromAircraftDepotWndPt(w, e.pt.x, e.pt.y, vp) == 0 && vp[0] != null
					&& sel == vp[0].index) {
			...
			case 4:
				if (!BitOps.HASBIT(w.disabled_state, 4) && w.as_traindepot_d().sel != Vehicle.INVALID_VEHICLE) {
					Vehicle v;
					...
					if (!Cmd.DoCommandP(v.tile, v.index, 0, null,
						Cmd.CMD_SELL_AIRCRAFT | Cmd.CMD_MSG(Str.STR_A01C_CAN_T_SELL_AIRCRAFT)))
						Global._backup_orders_tile = null;
				}
				break;
			default:
	...
	static final Widget _aircraft_depot_widgets[] = {
		new Widget(Window.WWT_CLOSEBOX, Window.RESIZE_NONE, 14, 0, 10, 0, 13, Str.STR_00C5, Str.STR_018B_CLOSE_WINDOW),
		new Widget(Window.WWT_CAPTION, Window.RESIZE_RIGHT, 14, 11, 318, 0, 13, Str.STR_A002_AIRCRAFT_HANGAR,
			Str.STR_018C_WINDOW_TITLE_DRAG_THIS),
		...
		new Widget(Window.WWT_IMGBTN, Window.RESIZE_LRTB, 14, 296, 318, 14, 61, 0x2A9,
			Str.STR_A023_DRAG_AIRCRAFT_TO_HERE_TO),

		new Widget(Window.WWT_MATRIX, Window.RESIZE_RB, 14, 0, 295, 14, 61, 0x204,
			Str.STR_A021_AIRCRAFT_CLICK_ON_AIRCRAFT),
		new Widget(Window.WWT_SCROLLBAR, Window.RESIZE_LRB, 14, 319, 330, 14, 61, 0x0,
			Str.STR_0190_SCROLL_BAR_SCROLLS_LIST),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_TB, 14, 0, 105, 62, 73, Str.STR_A003_NEW_AIRCRAFT,
			Str.STR_A022_BUILD_NEW_AIRCRAFT),
		new Widget(Window.WWT_NODISTXTBTN, Window.RESIZE_TB, 14, 106, 212, 62, 73, Str.STR_CLONE_AIRCRAFT,
			Str.STR_CLONE_AIRCRAFT_INFO_HANGAR_WINDOW),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_TB, 14, 213, 318, 62, 73, Str.STR_00E4_LOCATION,
			Str.STR_A024_CENTER_MAIN_VIEW_ON_HANGAR),
	...
	static final WindowDesc _aircraft_depot_desc = new WindowDesc(-1, -1, 331, 74, Window.WC_VEHICLE_DEPOT, 0,
		WindowDesc.WDF_STD_TOOLTIPS | WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET
			| WindowDesc.WDF_UNCLICK_BUTTONS | WindowDesc.WDF_STICKY_BUTTON | WindowDesc.WDF_RESIZABLE,
		_aircraft_depot_widgets, AirCraft::AircraftDepotWndProc);
	...
	static final Widget _player_aircraft_widgets[] = {
		new Widget(Window.WWT_CLOSEBOX, Window.RESIZE_NONE, 14, 0, 10, 0, 13, Str.STR_00C5, Str.STR_018B_CLOSE_WINDOW),
		new Widget(Window.WWT_CAPTION, Window.RESIZE_RIGHT, 14, 11, 247, 0, 13, Str.STR_A009_AIRCRAFT,
			Str.STR_018C_WINDOW_TITLE_DRAG_THIS),
		new Widget(Window.WWT_STICKYBOX, Window.RESIZE_LR, 14, 248, 259, 0, 13, 0x0, Str.STR_STICKY_BUTTON),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_NONE, 14, 0, 80, 14, 25, Str.SRT_SORT_BY,
			Str.STR_SORT_ORDER_TIP),
		new Widget(Window.WWT_PANEL, Window.RESIZE_NONE, 14, 81, 235, 14, 25, 0x0, Str.STR_SORT_CRITERIA_TIP),
		new Widget(Window.WWT_TEXTBTN, Window.RESIZE_NONE, 14, 236, 247, 14, 25, Str.STR_0225,
			Str.STR_SORT_CRITERIA_TIP),
		new Widget(Window.WWT_PANEL, Window.RESIZE_RIGHT, 14, 248, 259, 14, 25, 0x0, Str.STR_NULL),
		new Widget(Window.WWT_MATRIX, Window.RESIZE_RB, 14, 0, 247, 26, 169, 0x401,
			Str.STR_A01F_AIRCRAFT_CLICK_ON_AIRCRAFT),
		new Widget(Window.WWT_SCROLLBAR, Window.RESIZE_LRB, 14, 248, 259, 26, 169, 0x0,
			Str.STR_0190_SCROLL_BAR_SCROLLS_LIST),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_TB, 14, 0, 124, 170, 181, Str.STR_A003_NEW_AIRCRAFT,
			Str.STR_A020_BUILD_NEW_AIRCRAFT_REQUIRES),
		new Widget(Window.WWT_PUSHTXTBTN, Window.RESIZE_TB, 14, 125, 247, 170, 181, Str.STR_REPLACE_VEHICLES,
			Str.STR_REPLACE_HELP),
	...
	static void PlayerAircraftWndProc(Window w, WindowEvent e) {
		...
		case WE_CLICK: {
			...
			case 4:
			case 5:/* Select sorting criteria dropdown menu */
				Window.ShowDropDownMenu(w, VehicleGui._vehicle_sort_listing, vl.sort_type, 5, 0, 0);
				return;

			case 7: { /* Matrix to show vehicles */
				int id_v = (e.pt.y - VehicleGui.PLY_WND_PRC__OFFSET_TOP_WIDGET)
					/ VehicleGui.PLY_WND_PRC__SIZE_OF_ROW_BIG;
...

				{
					...
					if (id_v >= vl.list_length)
						return; // click out of list bound
		...
		case WE_TICK: /* resort the list every 20 seconds orso (10 days) */
			if (--vl.resort_timer == 0) {
				Global.DEBUG_misc(1, "Periodic resort aircraft list player %d station %d", owner, station);
				vl.resort_timer  = Global.DAY_TICKS * VehicleGui.PERIODIC_RESORT_DAYS;
				vl.flags        |= Vehicle.VL_RESORT;
				w.SetWindowDirty();
			}
	...
	static final WindowDesc _player_aircraft_desc = new WindowDesc(-1, -1, 260, 182, Window.WC_AIRCRAFT_LIST, 0,
		WindowDesc.WDF_STD_TOOLTIPS | WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET
			| WindowDesc.WDF_UNCLICK_BUTTONS | WindowDesc.WDF_STICKY_BUTTON | WindowDesc.WDF_RESIZABLE,
		_player_aircraft_widgets, AirCraft::PlayerAircraftWndProc);

	static final WindowDesc _other_player_aircraft_desc = new WindowDesc(-1, -1, 260, 182, Window.WC_AIRCRAFT_LIST, 0,
		WindowDesc.WDF_STD_TOOLTIPS | WindowDesc.WDF_STD_BTN | WindowDesc.WDF_DEF_WIDGET
			| WindowDesc.WDF_UNCLICK_BUTTONS | WindowDesc.WDF_STICKY_BUTTON | WindowDesc.WDF_RESIZABLE,
		_other_player_aircraft_widgets, AirCraft::PlayerAircraftWndProc);
